...
<body>
  ...
  <div class="container">
    ...
    <div class="card"><i class="fas fa-biohazard" style="color:var(--accent)"></i><div class="label">Air Purity</div><div class="value"><span id="gas">0</span></div></div>
  ...
  <div class="features-section">
    ...
    <div class="feature-grid">
      ...
      <div class="feature-item"><i class="fas fa-leaf"></i> Eco-Logic Energy Saver</div>
...
void loop() {
  ...
  // --- GAS HAZARD LOGIC ---
  if (gasValue > 800) { // Threshold for LPG/Smoke
    serialMirror = "⚠️ GAS HAZARD DETECTED! Evacuate Now!";
    doorStatus = "Open"; doorServo.write(90); // Open door for safety exit
    // Turn on all lights for clear path
    digitalWrite(room1[0], HIGH); digitalWrite(room1[1], HIGH); r1_Status = "ON";
    digitalWrite(room2[0], HIGH); digitalWrite(room2[1], HIGH); r2_Status = "ON";
  } else {
    // --- ROOM LIGHTS & ENERGY SAVER MODE ---
    if (count == 0) {
      // ENERGY SAVER: No one home, keep lights OFF
      digitalWrite(room1[0], LOW); digitalWrite(room1[1], LOW); r1_Status = "OFF";
      digitalWrite(room2[0], LOW); digitalWrite(room2[1], LOW); r2_Status = "OFF";
    } else {
      // Occupants present: Automatic LDR logic
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    ...
    id.toUpperCase();
    if (id == "C4AB6A05") { // Authorized Card
      ...
      serialMirror = "ACCESS DENIED"; doorStatus = "Denied"; 
    }
    rfid.PICC_HaltA(); rfid.PCD_StopCrypto1();
  }

  // --- IR SENSOR LOGIC (ENTRY/EXIT) ---
